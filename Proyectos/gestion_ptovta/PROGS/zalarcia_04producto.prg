...
SCAN FOR !EOF()
	...
	ELSE
		LOCATE FOR 'ALARCIA'$cnombre AND ctaacreedor = 1
    		IF 'ALARCIA'$cnombre AND ctaacreedor = 1
    			lnidctacte = Csrctacte.id
    		ENDIF 
	ENDIF
	...
	lnidforma = IIF(RECCOUNT('FsrForma')#0,FsrForma.id,1)
    	...
    	lnidestado = 1 &&IIF(empty(FsrArticulo.debaja),1,2)
	...
	ENDIF 		
	ldfecmodi	= ldfecha
	IF !EMPTY(FsrArticulo.fecha)
		ldfecmodi = DATETIME(YEAR(FsrArticulo.fecha),MONTH(FsrArticulo.fecha),DAY(FsrArticulo.fecha),0,0,0)
	ENDIF 
	ldfecoferta	= ldfecha		
	...
	lnidctacpra = 0&&FsrArticulo.cuenta
	lnidctavta = 0 &&FsrArticulo.cuenta
