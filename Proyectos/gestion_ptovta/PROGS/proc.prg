...
*====================================
function lClaseContenedoraGrid
...
return inlist(lcBC,'CONTAINER','PAGEFRAME','PAGE','FORM','GRID')


function ObjectFormat
parameters toContenedor,cBaseClass,nRed,nGreen,nBlue,cClave

cBaseClass = IIF(PCOUNT()<2,"Label",cBaseClass)
nRed = IIF(PCOUNT()<3,255,nRed)
nGreen = IIF(PCOUNT()<4,255,nGreen)
nBlue = IIF(PCOUNT()<5,255,nBlue)
cClave = IIF(PCOUNT()<6,"FORECOLOR",UPPER(cClave))

DO CASE
CASE upper(toContenedor.baseclass)='GRID'
	IF "FORECOLOR"$cClave
		toContenedor.HighLightForeColor = RGB(nRed,nGreen,nBlue)
		toContenedor.ForeColor = RGB(nRed,nGreen,nBlue)
	ENDIF 
	IF "HIGHLIGHTFORECOLOR"$cClave
		toContenedor.HighLightForeColor = RGB(nRed,nGreen,nBlue)
	ENDIF 
	IF "BACKCOLOR"$cClave
		*toContenedor.HighLightBackColor = RGB(nRed,nGreen,nBlue)
		toContenedor.BackColor = RGB(nRed,nGreen,nBlue)
	ENDIF 
	IF "HIGHLIGHTBACKCOLOR"$cClave
		toContenedor.HighLightBackColor = RGB(nRed,nGreen,nBlue)
	ENDIF 
	IF "SELECTEDFORECOLOR"$cClave
		toContenedor.SelectedItemForeColor = RGB(nRed,nGreen,nBlue)
	ENDIF 
	IF "SELECTEDBACKCOLOR"$cClave
		toContenedor.SelectedItemBackColor = RGB(nRed,nGreen,nBlue)
	ENDIF 
	FOR EACH loColumns IN toContenedor.Columns
		=ObjectFormat(loColumns,cBaseClass,nRed,nGreen,nBlue,cClave)
	ENDFOR 
OTHERWISE
	FOR EACH loControl IN toContenedor.Controls
		IF UPPER(loControl.BaseClass)$UPPER(cBaseClass)
			IF "FORECOLOR"$cClave
				loControl.ForeColor = RGB(nRed,nGreen,nBlue)
			ENDIF 
			IF "DISABLEDFORECOLOR"$cClave
				loControl.DisabledForeColor = RGB(nRed,nGreen,nBlue)
			ENDIF 
			IF "BACKCOLOR"$cClave
				loControl.BackColor = RGB(nRed,nGreen,nBlue)
			ENDIF 
			IF "DISABLEDBACKCOLOR"$cClave
				loControl.DisabledBackColor = RGB(nRed,nGreen,nBlue)
			ENDIF 
			IF "SELECTEDFORECOLOR"$cClave
				loControl.SelectedForeColor = RGB(nRed,nGreen,nBlue)
			ENDIF 
			IF "SELECTEDBACKCOLOR"$cClave
				loControl.SelectedBackColor = RGB(nRed,nGreen,nBlue)
			ENDIF 
		ENDIF 
		*-- LLAMADA RECURSIVA PARA CONTROLES DE TIPO CONTENEDOR
		if lClaseContenedoraGrid(loControl)      
			*-- PARA PAGEFRAMES, LLAMAR POR CADA PAGINA, YA QUE NO TIENE PROPIEDAD CONTROLS
			...
			CASE upper(loControl.baseclass)='PAGEFRAME'
				FOR EACH loPages IN loControl.Pages
					=ObjectFormat(loPages,cBaseClass,nRed,nGreen,nBlue,cClave)
				ENDFOR 
			CASE upper(loControl.baseclass)='GRID'
				FOR EACH loColumns IN loControl.Columns
					=ObjectFormat(loColumns,cBaseClass,nRed,nGreen,nBlue,cClave)
				ENDFOR 
			OTHERWISE 
				=ObjectFormat(loControl,cBaseClass,nRed,nGreen,nBlue,cClave)		
			ENDCASE 
		endif
	ENDFOR 
ENDCASE
...
*----------------------------------------------------------
* Funcion que convierte a mayusculas. Implentada para la Ñ
*----------------------------------------------------------
FUNCTION MaySTR
PARAMETERS lcnombre
LOCAL lni,lc
...
IF lok
   ...
   Goapp.idareaneg	= IIF(ISNULL(CsrSeteotermi.idareaneg),0,CsrSeteoTermi.idareaneg)
ENDIF
...
TRY 
	DO case
		CASE LcDataSourceType='ADO' OR LcDataSourceType='ODBC'
		    ...
		    CATCH
		    	=Oavisar.usuario('Falta el OCX del objeto pgbole',0)					                              			    
		    endtry
