...
lRutaSemanal = .f. &&Determina si se crea una guia de visita por dia o para toda la semana
...
SCAN FOR !EOF()
	...
	cRuta = LEFT("VDOR "+strtrim(CsrVendedor.numero)+" "+strtrim(CsrZona.numero)+" "+TRIM(Csrzona.nombre),30)	
	SELECT CsrRuta
	LOCATE FOR nombre=cRuta
	IF nombre#cRuta
		INSERT INTO CsrRuta (id,numero,nombre) ;
		VALUES (lnid,lnNumRuta,cRuta)		     		
		lnid = lnid + 1
	...
	lcdias = IIF(lRutaSemanal,'234567','2')
