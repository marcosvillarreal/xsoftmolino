...
TEXT TO lcCmd TEXTMERGE NOSHOW 
select convert(varchar(6),c.fecha,112) as periodo
...
Where c.tipoasi='C'
group by  convert(varchar(6),c.fecha,112),debehaber,p.cuenta
order by  convert(varchar(6),c.fecha,112),debehaber
ENDTEXT 
...
DO WHILE !EOF()
	...
	ldfecha			= GOMONTH(CTOD(StoC(STR(CsrCabeza.periodo,6)+'01')),1)-1
