...
**OR tipocomp=5
SELECT distinct cliente+10000 as cliente,orden,fecha_fac;
...
,IIF(isnull(perceibrn+perceib),CAST(0 as numeric(11,2)),perceibrn+perceib) as TotalPB;
...
,IIF(isnull(perceibrn),CAST(0 as numeric(11,2)),perceibrn) as TotalPBRN;
...
SELECT cliente+10000 as cliente,IIF(provincia='R',0,0) as idprovincia;
...
,AVG(porceniva) as Tasa,SUM(perceibrn) as TotalPBRN, SUM(perceib) as totalPBBA;
FROM Csrmoviprov;
LEFT JOIN CsrComproba ON CsrMoviprov.tipocomp=CsrComproba.numero;
...
SELECT distinct Csrmoviprov.cliente+10000 as cliente;
...
SELECT (CsrMoviprov.cliente+10000) as cliente ,detalle,IIF((CsrComproba.debcre='C'),-1,01) as Signo;
,fecha_fac;
...
,(perceibrn+perceib) as TotalPB,percegan as TotalPG,percep as TotalPI;
,IIF(provincia='R',1100000022,1100000002) as idprovincia;
,(perceibrn) as TotalPBRN,(perceib) as TotalPBBA;
...
SCAN  
	...
	IF FOUND()
		*ldfechas=DATETIME(YEAR(CsrPosterior.fecha),month(CsrPosterior.fecha),day(CsrPosterior.fecha),0,0,0)
		...
		,lnentrega,ldfechavto,lnimporte,"Comp.Importado - "+lcdetalle,lcperiodo,"0000",lnsigno)
