...
RETURN lccuit
*-----------------------------------------------------------------------------
*FUNCTION LeyendaCtacte(nIdComprobante,cCtaResta,cCtaSuma)
*-----------------------------------------------------------------------------
FUNCTION LeyendaCtacte
PARAMETERS nIdComprobante,cCtaResta,cCtaSuma
LOCAL cLeyenda

cLeyenda = "No afecta deuda"
DO CASE
CASE STR(nidComprobante,10)$cCtaResta
	cLeyenda = "Resta deuda"
CASE STR(nidComprobante,10)$cCtaSuma
	cLeyenda = "Aumenta deuda"
ENDCASE
RETURN cLeyenda

*-----------------------------------------------------------------------------
*FUNCTION ValidarCPostal(lcCodPostal)
*-----------------------------------------------------------------------------
FUNCTION ValidarCPostal
PARAMETERS lcCodPostal

lcCodPostal = ALLTRIM(lcCodPostal)
IF LEN(lcCodPostal) < 8
	&&Eliminamos la letra y solo dejamos los siguientes 4
	IF NOT ISALPHA(LEFT(lcCodPostal,1)) &&No es letra, le adjuntamos un espacio
		lcCodPostal = " "+lcCodPostal
	ENDIF 
	lcCodPostal = " "+RIGHT(LEFT(lcCodPostal,5),4)
ENDIF 
RETURN lcCodPostal
...
TEXT TO lcCmd TEXTMERGE NOSHOW 
SELECT CsrParaVario.* FROM paravario as CsrParavario WHERE idorigen=<<Goapp.terminal>>
and nombre = 'REPORTE<<strzero(Goapp.terminal,4)>>'
ENDTEXT 
lok =CrearCursorAdapter("CsrParaVario",lcCmd)
IF lok
...
Catch To loCatchErr
	...
	=Oavisar.usuario('La conexión a la Base de Datos a fallado'+CHR(13);
					+CHR(13)+IIF(lldesarrollo,lcConectionString,""),0)
	RETURN .f.
ENDTRY 
