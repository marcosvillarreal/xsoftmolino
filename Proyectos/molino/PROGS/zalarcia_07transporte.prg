...
USE ALLTRIM(lcpath )+"\CEREALES\camiones" in 0 alias FsrCamion EXCLUSIVE
...
SCAN FOR !EOF()
	IF numero=0
      		LOOP 
	ENDIF
	lcnombre = mayStr(FsrChofer.nombre)
...
SCAN FOR !EOF()
	IF EMPTY(numero)
      		LOOP 
      	ENDIF
      	lcnombre = mayStr(FsrCamion.nombre)
      	lcpatente = LTRIM(FsrCamion.numero)
      	lnnumero = lnnumero+1
      	lcmarca  = mayStr(FsrCamion.marca)
      	lccolor	 = mayStr(FsrCamion.color)
      	lntara 	 = FsrCamion.tara
      	lcswitch = '00001'
      	lnidctacte = 0
      	SELECT CsrCtacte
      	LOCATE FOR cnumero=LTRIM(STR(FsrCamion.empresa))
      	IF cnumero=LTRIM(STR(FsrCamion.empresa))
      		lnidctacte = CsrCtacte.id
      	ENDIF 
      	lnidchofer = 0
      	SELECT CsrChofer
      	LOCATE FOR numero = FsrCamion.chofer
      	IF numero = FsrCamion.chofer
