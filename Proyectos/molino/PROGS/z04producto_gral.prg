...
SCAN FOR !EOF()
    ...
    lncuenta	= FsrSeccion.cuentavta &&Guardamos la cuenta contable
                         
    INSERT INTO CsrRubro (id,numero,nombre,idtipoprod,idtipovta,perceibruto,idfuerzavta,nolista;
    ,porcecomi,porcesuge,porcedev,tasavied,tasapata,esharina,cuenta_no) ;
    VALUES (lnid,lnnumero,lcnombre,lntipoprod,lntipovta,lnretibruto,lnidfrzvta,lnolista;
    ,lnporcecomi,lnporcesuge,lnporcedev,lntasavied,lntasapata,0,lncuenta)
    lnid = lnid + 1
...
SCAN FOR !EOF()
    ...
    SELECT CsrCtacte
    LOCATE FOR VAL(cnumero)=977
    IF FOUND()
    	lnidctacte = Csrctacte.id
    ELSE
    	lnidctacte =  CsrParametros.idorigen
    ENDIF
	
	lnidctacpra = DefaultVar('FsrArticulo.cuenta',0)
	lnidctavta  = DefaultVar('FsrArticulo.cuenta',0)
	
    SELECT CsrRubro
    LOCATE FOR numero=Fsrarticulo.seccion
    IF FOUND()
        lnidrubro = CsrRubro.id
        IF CsrRubro.idtipoprod = 0
        	replace idtipoprod WITH lnidctavta IN CsrRubro
        	lnidctavta = 0
        ENDIF 
    ENDIF
    ...
    lnidestado 		= 1
    DO CASE 
    CASE Fsrarticulo.tablaiva=2 
    	lnidiva        	= 1100000003
    CASE Fsrarticulo.tablaiva=3
    	lnidiva        	= 1100000001
    OTHERWISE 
    	lnidiva        	= 1100000002
    ENDCASE 
    ...
    lnvtakilos		= 0
    lnsireparto		= 0
   	lnpeso			= DefaultVar('FsrArticulo.peso',0)     
	...
	lnmargen1 = FsrArticulo.utilidad&&IIF(FsrArticulo.utilidad>999.99,999.99,FsrArticulo.utilidad)
	lnmargen2 = FsrArticulo.utilidad2&&IIF(FsrArticulo.utilidad2>999.99,999.99,FsrArticulo.utilidad2)
	lnmargen3 = FsrArticulo.utilidad3&&IIF(FsrArticulo.utilidad3>999.99,999.99,FsrArticulo.utilidad3)
	...
	lnppt			= DefaultVar('FsrArticulo.itc',0) 
	lnitc			= IIF(EMPTY(DefaultVar('FsrArticulo.leyenda',"") ),0,(IIF(ALLTRIM(DefaultVar('FsrArticulo.leyenda',""))='I',1,2)))
	...
	lninternoporce    = red((lninterno/lncosto)*100,3)
	
	IF DefaultVar('FsrArticulo.ctayorden',"")$'S'
		lnctaaorden = 1
	ENDIF 
	...
	*Una vez caragdo todos los productos, recorremos los que idenvase#0 y buscamos el articulo = envase.
	...
	IF DefaultVar('FsrArticulo.perceib',"")$'S' OR DefaultVar('FsrArticulo.llevaib',"")$'S' OR DefaultVar('FsrArticulo.piblp',"")$'S'
		SELECT CsrProvincia
		GO TOP 
		SCAN 
			IF  numero = 2 OR numero=10 OR numero = 22
